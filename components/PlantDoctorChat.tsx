...
const PlantDoctorChat: React.FC<PlantDoctorChatProps> = ({ language, userProfile, initialMode }) => {
  ...
  const getInitialMessageText = (mode?: string) => {
      ...
      if (mode === 'soil') {
          return `Hello ${userProfile.name}! Ready to check your soil health? ⛰️\n\nPlease **upload a clear photo of your soil** (held in hand or on the ground). I will analyze its texture and suggest improvements.`;
      }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (imageToSend) {
        if (currentMode === 'soil') {
            queryForAI = `[Soil Analysis Request] The user has uploaded an image of their soil. 
            Please analyze the visible physical properties (texture, color, potential organic matter).
            Based on the User Profile (${userProfile.role}, ${userProfile.location}), suggest:
            1. Suitability for common crops/plants.
            2. Organic amendments to improve structure.
            User Query: ${input}`;
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {messages.map((msg) => (
          ...
          >
            <div className={`flex max-w-[90%] md:max-w-[85%] gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                {/* Avatar */}
                <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center shadow-sm border mt-auto mb-1 ${
                  msg.role === 'user' 
                    ? 'bg-green-50 border-green-200 text-green-700 dark:bg-green-900/50 dark:border-green-800 dark:text-green-400' 
                    : 'bg-white border-gray-200 text-emerald-600 dark:bg-gray-800 dark:border-gray-700 dark:text-emerald-400'
                }`}>
                ...
                {/* Message Bubble */}
                <div
                  className={`flex flex-col rounded-2xl p-4 text-[15px] leading-relaxed relative transition-all shadow-sm ${
                    msg.role === 'user'
                      ? 'bg-green-50/50 dark:bg-green-900/20 border border-green-100 dark:border-green-800/50 text-gray-800 dark:text-gray-100 rounded-br-none' 
                      : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100 rounded-bl-none'
                  }`}
                >
                  {msg.image && (
                    <div className="mb-3 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm bg-white dark:bg-gray-900">
                      <img 
                        src={msg.image} 
                        alt="User upload" 
                  ...
                  <div className={`prose prose-sm max-w-none break-words ${
                      msg.role === 'user' 
                      ? 'dark:prose-invert prose-p:text-gray-800 dark:prose-p:text-gray-100 prose-a:text-green-700 dark:prose-a:text-green-400' 
                      : 'dark:prose-invert prose-p:text-gray-800 dark:prose-p:text-gray-100 prose-headings:font-bold prose-h3:text-green-700 dark:prose-h3:text-green-400 prose-strong:font-bold prose-strong:text-gray-900 dark:prose-strong:text-white'
                  }`}>
                    <ReactMarkdown 
                        components={{
                  ...
                  {msg.groundingUrls && msg.groundingUrls.length > 0 && (
                    <div className={`mt-3 pt-3 border-t flex flex-col gap-2 ${
                        msg.role === 'user' ? 'border-green-200 dark:border-green-800/50' : 'border-gray-100 dark:border-gray-700'
                    }`}>
                      <p className="text-[10px] font-bold opacity-70 uppercase tracking-wider flex items-center gap-1">
                        Sources
                      </p>
                        ...
                        {msg.groundingUrls.map((url, idx) => (
                          <a 
                            ...
                            className={`text-xs px-2.5 py-1.5 rounded-lg transition-all flex items-center gap-1.5 max-w-full group ${
                                msg.role === 'user'
                                ? 'bg-green-100/50 dark:bg-green-900/50 hover:bg-green-100 dark:hover:bg-green-900 text-green-800 dark:text-green-200 border border-green-200 dark:border-green-800'
        ...
        {loading && (
          <div className="flex justify-start w-full animate-fade-in">
            <div className="flex max-w-[90%] md:max-w-[85%] gap-3 flex-row items-end">
                 <div className="flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center bg-white border border-gray-100 text-emerald-600 dark:bg-gray-800 dark:border-gray-700 dark:text-emerald-400 shadow-sm mb-1">
                   <Leaf size={14} className="animate-pulse" />
                 </div>
                 <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl rounded-bl-none p-4 flex items-center gap-3 shadow-sm">
