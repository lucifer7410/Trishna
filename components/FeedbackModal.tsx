...
const FeedbackModal: React.FC<FeedbackModalProps> = ({ onClose, userEmail }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const storedProfile = localStorage.getItem('trishna_user_profile');
        if (storedProfile) {
    ...
    try {
        await sendFeedbackEmail(userEmail || 'unknown@trishna.app', name, type, message, rating);
    } catch (error) {
    ...
    return (
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-sm w-full text-center animate-scale-in">
          ...
          <p className="text-gray-500 dark:text-gray-400">Your feedback has been sent to our team.</p>
        </div>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
          <div className="space-y-6">
            ...
            <button
                ...
                className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
            >
