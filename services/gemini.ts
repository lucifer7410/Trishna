...
// Retrieve API Key from Environment Variables with Fallbacks
const getApiKey = () => {
...
// Lazy initialization pattern to prevent top-level crashes
...
const getAi = (): GoogleGenAI => {
  if (!aiInstance) {
    ...
    // If key is missing, we pass a dummy string to allow the app to load (calls will fail gracefully with 400s).
    aiInstance = new GoogleGenAI({ apiKey: apiKey || 'dummy_key_missing' }); 
  }
  return aiInstance;
};
...
// Check for Authentication/Key Errors (400, 403)
const isAuthError = (error: any): boolean => {
    const msg = getErrorMessage(error);
    const status = error?.status || error?.response?.status;
    
    // Google GenAI usually returns 400 for invalid API keys
    return status === 400 || 
           status === 403 || 
           (typeof msg === 'string' && (
...
export const fetchWeatherAndAlerts = async (location: string, language: string): Promise<WeatherData> => {
  const key = getApiKey();
  if (!key) return getMockWeather(location);
  ...
  try {
    ...
    return {
     ...
     if (isAuthError(error)) {
        // Explicit instruction for Netlify users
        return {
            text: "⚠️ Authentication Error: API Key invalid or missing. If on Netlify, please add VITE_API_KEY in Site Settings > Environment Variables and redeploy.",
            groundingUrls: []
        };
     }
     if (isQuotaError(error)) {
         return {
            text: "I am currently experiencing very high traffic. Please try again in a few minutes, or check our offline guides in the 'Learn' section.",
            groundingUrls: []
        };
     }
